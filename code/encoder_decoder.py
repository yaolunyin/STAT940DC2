    ...
    def __init__(self, tokenizer, args, file_path, block_size=512):
        ...
        if os.path.exists(cached_features_file) and not args.overwrite_cache:
            ...
            max_input_length = min(
                args.max_input_length, max([len(ex) for ex in inputs_old])
            )
            max_output_length = min(
                args.max_output_length, max([len(ex) for ex in outputs_old])
            )

            input_lengths = [min(len(ex), max_input_length) for ex in inputs_old]
            output_lengths = [min(len(ex), max_output_length) for ex in outputs_old]

            inputs = [tokenizer.encode(
                ex[0], add_special_tokens=False, max_length=max_input_length, padding="max_length")
                for ex in examples]

            outputs = [[outputs_old[i][0]] + 
              tokenizer.encode(
                ex[1], add_special_tokens=False, max_length=max_output_length, padding="max_length")
                for i, ex in enumerate(examples)]

            
            # print("one of the inputs before pad: ", inputs_old[0])
            # print("one of the inputs after pad: ", inputs[0])
            # print("one of the outputs before pad: ", outputs_old[0])
            # print("one of the outputs after pad: ", outputs[0])
            # exit()
